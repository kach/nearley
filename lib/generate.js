(function(root, factory) {
    ...
    var generate = function (parser, exportName) {
        ...
        const lang = parser.config.preprocessor[0];
        const args = parser.config.preprocessor.slice(1);

        if(!generate[lang]) {
            throw new Error("No such preprocessor: " + lang)
        }

        return generate[lang](parser, exportName, args);
    };
    ...
    generate.ts = generate.typescript = function (parser, exportName, args) {
        const map = {};
        const validNames = ["lexer", "token"];
        for (const a of args) {
            if (map[a[0]] !== undefined) {
                throw new Error(`Duplicate name "${a[0]}" after directive`);
            } else if (!validNames.includes(map[a[0]])) {
                throw new Error(`"${a[0]}" not a valid name; must be ${validNames.map(n => '"' + n + '"').join(", ")}`)
            }
            map[a[0]] = a[1].literal;
        };
        var output = "// Generated automatically by nearley, version " + parser.version + "\n";
        ...
        output += parser.body.join('\n');
        output += "\n";
        if (map["token"] === undefined && map["lexer"] === undefined) {
        ...
        output += "  Lexer: " + (map["lexer"] ?? "NearleyLexer") + " | undefined;\n";
