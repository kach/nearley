...
function serializeRules(rules, specialPostprocessors) {
    return "[\n    " + rules.map(function(rule) {
      return serializeRule(rule, specialPostprocessors);
    }).join(",\n    ") + "\n]";
}
...
function serializeRule(rule, specialPostprocessors) {
    var ret = '{';
    ret += '"name": ' + JSON.stringify(rule.name);
    ret += ', "symbols": [' + rule.symbols.map(serializeSymbol).join(', ') + ']';
    if (rule.postprocess) {
        if(rule.postprocess.special) {
            rule.postprocess = specialPostprocessors[rule.postprocess.special];
        }
...
generate.js = generate._default = generate.javascript = function (parser, exportName) {
    ...
    output += "var grammar = {\n";
    output += "    ParserRules: " +
        serializeRules(parser.rules, generate.javascript.specialPostprocessors)
        + "\n";
...
generate.javascript.specialPostprocessors = {
...
generate.cs = generate.coffee = generate.coffeescript = function (parser, exportName) {
    ...
    output += "    ParserRules: " +
        tabulateString(
            serializeRules(parser.rules, generate.coffeescript.specialPostprocessors),
            '      ',
            id)
        + ",\n";
    output += "    ParserStart: " + JSON.stringify(parser.start) + "\n";
...
generate.coffeescript.specialPostprocessors = {
    "joiner": "(d) -> d.join('')",
