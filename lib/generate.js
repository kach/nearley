(function(root, factory) {
    ...
    function serializeRules(rules, builtinPostprocessors, minifyRuleNames) {
        return "[\n    " + rules.map(function(rule) {
          return serializeRule(rule, builtinPostprocessors, minifyRuleNames);
        }).join(",\n    ") + "\n]";
    }
    ...
    function serializeSymbol(s, minifyRuleNames) {
        if (s instanceof RegExp) {
            return s.toString();
        } else if (s.token) {
            return s.token;
        } else {
            return toCommentedShortName(s, minifyRuleNames);
        }
    }

    var ruleNameLookup = {}
    var ruleNameCounter = 0
    function shortenName(name) {
        if (!ruleNameLookup[name]) {
            ruleNameLookup[name] = '' + ruleNameCounter++;
        }
        return ruleNameLookup[name];
    }

    function toCommentedShortName(name, minifyRuleNames) {
        if (minifyRuleNames && typeof name === 'string' && name.indexOf('$') >= 0) {
    ...
    function serializeRule(rule, builtinPostprocessors, minifyRuleNames) {
        var ret = '{';
        ret += '"name": ' + toCommentedShortName(rule.name, minifyRuleNames);
        ret += ', "symbols": [' + rule.symbols.map(s => serializeSymbol(s, minifyRuleNames)).join(', ') + ']';
        if (rule.postprocess) {
    ...
    var generate = function (parser, opts) {
        ...
        ruleNameLookup = {};
        ruleNameCounter = 0;
        return generate[parser.config.preprocessor](parser, opts);
    };

    generate.js = generate._default = generate.javascript = function (parser, opts) {
        var exportName = opts.export
        var minifyRuleNames = opts['minify-rule-names']
        ...
        output += "    ParserRules: " +
            serializeRules(parser.rules, generate.javascript.builtinPostprocessors, minifyRuleNames)
            + "\n";
        output += "  , ParserStart: " + toCommentedShortName(parser.start, minifyRuleNames) + "\n";
        output += "}\n";
    ...
    generate.module = generate.esmodule = function (parser, opts) {
        var exportName = opts.export
        var minifyRuleNames = opts['minify-rule-names']
        var output = "// Generated automatically by nearley, version " + parser.version + "\n";
        output +=  "// http://github.com/Hardmath123/nearley\n";
        ...
        output += "let ParserRules = " + serializeRules(parser.rules, generate.javascript.builtinPostprocessors, minifyRuleNames) + ";\n";
        output += "let ParserStart = " + toCommentedShortName(parser.start, minifyRuleNames) + ";\n";
        output += "export default { Lexer, ParserRules, ParserStart };\n";
        return output;
    };

    generate.cs = generate.coffee = generate.coffeescript = function (parser, opts) {
        var exportName = opts.export
        var minifyRuleNames = opts['minify-rule-names']
        var output = "# Generated automatically by nearley, version " + parser.version + "\n";
        output +=  "# http://github.com/Hardmath123/nearley\n";
        ...
        output += "    ParserRules: " +
            tabulateString(
                    serializeRules(parser.rules, generate.coffeescript.builtinPostprocessors, minifyRuleNames),
                    '      ',
                    {indentFirst: false})
        + ",\n";
        output += "    ParserStart: " + toCommentedShortName(parser.start, minifyRuleNames) + "\n";
    ...
    generate.ts = generate.typescript = function (parser, opts) {
        var exportName = opts.export
        var minifyRuleNames = opts['minify-rule-names']
        var output = "// Generated automatically by nearley, version " + parser.version + "\n";
        output +=  "// http://github.com/Hardmath123/nearley\n";
        ...
        output += "\n";
        output += "export var ParserRules: NearleyRule[] = " + serializeRules(parser.rules, generate.typescript.builtinPostprocessors, minifyRuleNames) + ";\n";
        output += "\n";
        output += "export var ParserStart: string = " + toCommentedShortName(parser.start, minifyRuleNames) + ";\n";
        return output;
    };
