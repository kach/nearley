...
function State(rule, dot, reference) {
    this.rule = rule;
    this.dot = dot;
...
State.prototype.nextState = function(data) {
    var state = new State(this.rule, this.dot + 1, this.reference);
...
State.prototype.process = function(location, ind, table, grammar, addedRules) {
    if (this.dot === this.rule.symbols.length) {
...
function Parser(rules, start) {
    ...
    var rules = this.grammar.byName[grammar.start] || [];
    var addedRules = [];
...
Parser.prototype.advanceTo = function(n, addedRules) {
    ...
    while (w < this.table[n].length) {
        (this.table[n][w]).process(n, w, this.table, this.grammar, addedRules);
        w++;
    }
}
