...
function State(rule, expect, reference, data) {
    this.rule = rule;
    this.expect = expect;
...
State.prototype.nextState = function(data) {
    var newdata = this.data.slice(0);  // make a cheap copy of currentState's data
    newdata.push(data);                // append the passed data
    return new State(this.rule, this.expect + 1, this.reference, newdata);
};
...
State.prototype.COMPLETE = function(table, location) {
    ...
    while (w < table[this.reference].length) {
        x = (table[this.reference][w]).consumeNonTerminal(this.rule.name);
        if (x) {
...
State.prototype.PREDICT = function(table, location, rules, addedRules) {
    ...
    rules.forEach(function(r) {
        ...
        if (r.name === exp && addedRules.indexOf(r) === -1) {
            // Make a note that you've added it already, and
            // don't need to add it again; otherwise
            // left recursive rules are going to go into an infinite loop
            // by adding themselves over and over again.

            // If it's the null rule, however,
            // you don't do this because it affects the current table row,
            // so you might need it to be called again later.
            // Instead, I just insert a copy whose state has been advanced one position
            // (since that's all the null rule means anyway)
...
State.prototype.process = function(table, location, rules, addedRules) {
    if (this.isComplete()) {
        this.COMPLETE(table, location);
    } else {
        this.PREDICT(table, location, rules, addedRules);
    }
};
...
Parser.prototype.advanceTo = function(n, addedRules) {
    ...
    while (w < this.table[n].length) {
        (this.table[n][w]).process(this.table, n, this.rules, addedRules);
        w++;
    }
}
