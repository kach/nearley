...
State.prototype.addDerivation = function(left, right) {
    this.left = left;
    this.right = right;
    if (this.isComplete) {
...
State.prototype.finish = function() {
    if (this.rule.postprocess) {
        this.data = this.rule.postprocess.call(this, this.data);
    }
};
...
Column.prototype.add = function(rule, dot, reference, wantedBy) {
    var s = new State(rule, dot, reference, wantedBy);
    this.states.push(s);
    return s;
}
...
Column.prototype.predict = function(exp) {
    ...
    for (var i = 0; i < rules.length; i++) {
        ...
        this.add(r, 0, this.index, wantedBy);
    }
}

Column.prototype.complete = function(left, right) {
    var copy = this.add(left.rule, left.dot + 1, left.reference, left.wantedBy);
    copy.addDerivation(left, right);
}
    ...
    while (token = lexer.next()) {
        ...
        for (var w = scannable.length; w--; ) {
            ...
            if (expect.test ? expect.test(tokenOrCh) :
                ...
                // Add it
                var next = nextColumn.add(state.rule, state.dot + 1, state.reference, state.wantedBy);
                next.addDerivation(state, {data: tokenOrCh, token: token, isToken: true, reference: n - 1});
            }
        }
